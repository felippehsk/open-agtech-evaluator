interface InfoTipProps {
  text: string
  /** Show tooltip above or below the icon. Use "bottom" when near the top of the page to avoid clipping. */
...
export function InfoTip({ text, placement = 'top', className = '' }: InfoTipProps) {
  return (
    <span className={`group relative inline-flex cursor-help ${className}`}>
      ...
      <span
        role="tooltip"
        className={`pointer-events-none absolute left-1/2 z-[100] hidden max-w-[220px] -translate-x-1/2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-normal text-slate-700 shadow-lg group-hover:block dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 ${
