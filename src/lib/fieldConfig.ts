...
export const SECTION_FIELDS: Record<string, FieldConfig[]> = {
  identity: [
    ...
    fc('country_of_origin', 'Country of origin', 'single_select', COUNTRY_OPTIONS, { allowCustomOptions: true }),
  ...
  ingestion: [
    ...
    fc('unit_system_handling', 'Unit system handling', 'single_select', UNIT_SYSTEM, { allowCustomOptions: true }),
    fc('data_format_limitations', 'Data format limitations or issues noted', 'textarea', undefined, { maxLength: 500 }),
  ],
  processing_2a: [
    fc('spatial_filtering', 'Spatial filtering capabilities', 'multi_select', SPATIAL_FILTERING, { allowCustomOptions: true }),
    fc('temporal_filtering', 'Temporal filtering capabilities', 'multi_select', TEMPORAL_FILTERING, { allowCustomOptions: true }),
    fc('multi_layer_overlay', 'Multi-layer overlay capabilities', 'multi_select', MULTI_LAYER, { allowCustomOptions: true }),
    fc('coordinate_system_handling', 'Coordinate reference system handling', 'single_select', CRS_HANDLING, { allowCustomOptions: true }),
  ],
  processing_2b: [
    fc('yield_data_cleaning', 'Yield data cleaning capabilities', 'multi_select', YIELD_CLEANING, { allowCustomOptions: true }),
    fc('combine_calibration_adjustment', 'Combine calibration adjustment', 'multi_select', COMBINE_CALIBRATION, { allowCustomOptions: true }),
    ...
    fc('interpolation_methods', 'Interpolation methods available', 'multi_select', INTERPOLATION, { allowCustomOptions: true }),
    fc('on_farm_trial_analysis', 'On-farm trial / strip trial analysis', 'multi_select', ['A/B strip comparison', 'Randomized plot comparison', 'Statistical significance testing', 'Economic analysis of trial results', 'Not available', 'Unknown'], { allowCustomOptions: true }),
    fc('profit_loss_map', 'Profit / loss map generation & agronomic-financial link', 'multi_select', PROFIT_LOSS, { allowCustomOptions: true }),
    fc('crop_scouting_tools', 'Crop scouting tools', 'multi_select', CROP_SCOUTING, { allowCustomOptions: true }),
  ],
  processing_2c_livestock: [
    fc('individual_animal_records', 'Individual animal records', 'single_select', INDIVIDUAL_ANIMAL),
    fc('rfid_eid_integration', 'RFID / EID integration', 'multi_select', RFID_OPTIONS, { allowCustomOptions: true }),
    ...
    fc('traceability_birth_to_harvest', 'Traceability (birth to harvest)', 'multi_select', TRACEABILITY, { allowCustomOptions: true }),
  ],
  processing_2d_cea: [
    fc('climate_control_monitoring', 'Climate control monitoring', 'multi_select', CLIMATE_CONTROL, { allowCustomOptions: true }),
  ...
  processing_2e_financial: [
    ...
    fc('cost_tracking_granularity', 'Cost tracking granularity', 'multi_select', COST_GRANULARITY, { allowCustomOptions: true }),
    fc('integration_accounting', 'Integration with accounting software', 'multi_select', ACCOUNTING_INTEGRATION, { allowCustomOptions: true }),
  ],
  export: [
    ...
    fc('batch_export', 'Batch export capability', 'single_select', BATCH_EXPORT, { allowCustomOptions: true }),
    fc('time_to_first_map', 'Time to first map (time to value)', 'single_select', TIME_TO_MAP, { allowCustomOptions: true }),
    fc('report_generation', 'Report generation', 'multi_select', ['Field summary reports', 'Yield analysis reports', 'Profitability reports', 'Custom report builder', 'Not available', 'Unknown'], { allowCustomOptions: true }),
  ],
  integration: [
    fc('direct_integrations', 'Direct platform integrations', 'multi_select', DIRECT_INTEGRATIONS, { allowCustomOptions: true }),
    fc('api_availability', 'API availability', 'single_select', API_AVAILABILITY, { allowCustomOptions: true }),
    fc('isobus_compliance', 'ISOBUS / ISO 11783 compliance', 'single_select', ISOBUS_COMPLIANCE, { allowCustomOptions: true }),
    fc('oem_equipment_compatibility', 'OEM equipment compatibility', 'multi_select', OEM_EQUIPMENT, { allowCustomOptions: true }),
    fc('data_format_interoperability', 'Data format interoperability (subjective)', 'rating', undefined, { ratingLabels: ['Poor', 'Excellent'] }),
    fc('hardware_dependency', 'Hardware dependency', 'single_select', HARDWARE_DEPENDENCY, { allowCustomOptions: true }),
    fc('adapt_support', 'AgGateway ADAPT framework support', 'single_select', ADAPT_SUPPORT, { allowCustomOptions: true }),
  ],
  governance: [
    fc('data_ownership_policy', 'Data ownership policy', 'single_select', DATA_OWNERSHIP, { allowCustomOptions: true }),
    fc('data_portability', 'Data portability', 'single_select', DATA_PORTABILITY, { allowCustomOptions: true }),
    fc('data_deletion_policy', 'Data deletion policy', 'single_select', DATA_DELETION, { allowCustomOptions: true }),
    fc('data_sharing_third_parties', 'Data sharing with third parties', 'single_select', DATA_SHARING, { allowCustomOptions: true }),
    fc('ag_data_transparent', 'Ag Data Transparent certified', 'single_select', ADT_CERT, { allowCustomOptions: true }),
    fc('privacy_policy_url', 'Privacy policy URL', 'url'),
    fc('data_storage_location', 'Data storage location', 'multi_select', STORAGE_LOCATION, { allowCustomOptions: true }),
    fc('security_measures', 'Encryption / security measures described', 'multi_select', SECURITY_MEASURES, { allowCustomOptions: true }),
  ],
  access: [
    fc('multi_user_access', 'Multi-user access', 'single_select', ['Yes — specify how many', 'No', 'Unknown']),
    fc('user_roles', 'User roles / permission levels', 'multi_select', USER_ROLES, { allowCustomOptions: true }),
    fc('sharing_advisors', 'Sharing with advisors / agronomists', 'single_select', ['Yes', 'No', 'Partial', 'Unknown']),
    fc('offline_functionality', 'Offline functionality', 'single_select', OFFLINE_FUNCTIONALITY, { allowCustomOptions: true }),
    fc('mobile_app_available', 'Mobile app available', 'multi_select', MOBILE_APP, { allowCustomOptions: true }),
    fc('storage_quotas', 'Storage quotas or limits', 'single_select', STORAGE_QUOTAS, { allowCustomOptions: true }),
  ],
  pricing: [
    fc('pricing_model', 'Pricing model', 'multi_select', PRICING_MODEL, { allowCustomOptions: true }),
    fc('free_tier_available', 'Free tier available', 'single_select', ['Yes', 'No', 'Partial — describe', 'Unknown']),
    fc('free_tier_limitations', 'Free tier limitations (if applicable)', 'multi_select', FREE_TIER_LIMITATIONS, { allowCustomOptions: true }),
    fc('trial_period', 'Trial period', 'single_select', ['Yes — duration', 'No', 'Unknown']),
    fc('educational_pricing', 'Educational / student pricing', 'single_select', EDUCATIONAL_ACCESS, { allowCustomOptions: true }),
    fc('contract_requirements', 'Contract requirements', 'single_select', CONTRACT_REQUIREMENTS, { allowCustomOptions: true }),
    ...
    fc('cost_to_leave', 'Cost to leave / data retention after cancellation', 'single_select', COST_TO_LEAVE, { allowCustomOptions: true }),
  ],
  usability: [
    fc('onboarding_setup', 'Onboarding / setup experience', 'rating', undefined, { ratingLabels: ['Very difficult', 'Very easy'] }),
    fc('documentation_quality', 'Documentation quality', 'single_select', DOCUMENTATION_QUALITY, { allowCustomOptions: true }),
    fc('training_resources', 'Training resources available', 'multi_select', TRAINING_RESOURCES, { allowCustomOptions: true }),
    fc('customer_support_channels', 'Customer support channels', 'multi_select', SUPPORT_CHANNELS, { allowCustomOptions: true }),
    fc('support_languages', 'Support language', 'multi_select', SUPPORT_LANGUAGES, { allowCustomOptions: true }),
    fc('update_frequency', 'Update frequency', 'text', undefined, { placeholder: 'Describe how often updated' }),
  ],
  assessment: [
    fc('strongest_capability', 'Strongest capability', 'single_select', STRONGEST_WEAKEST_OPTIONS, { allowCustomOptions: true }),
    fc('strongest_capability_justification', 'Strongest capability — justification (max 200 chars)', 'textarea', undefined, { maxLength: 200 }),
    fc('weakest_capability', 'Weakest capability or biggest gap', 'single_select', STRONGEST_WEAKEST_OPTIONS, { allowCustomOptions: true }),
    fc('weakest_capability_justification', 'Weakest capability — justification (max 200 chars)', 'textarea', undefined, { maxLength: 200 }),
    fc('best_suited_for', 'Best suited for', 'multi_select', BEST_SUITED_FOR, { allowCustomOptions: true }),
  ...
  evidence: [
    fc('source_types_used', 'Source types used', 'multi_select', SOURCE_TYPES_USED, { allowCustomOptions: true }),
